...
const AuthView: React.FC<AuthViewProps> = ({ initialSignUp = false }) => {
  ...
  const handleSignUpSubmit = async (e: React.FormEvent) => {
    ...
    if (!username || !email || !password || !confirmPassword) {
      toast.showWarning(t('auth.register.errors.fillAll', { defaultValue: 'Vui lòng điền đầy đủ thông tin' }));
      return;
    }

    if (password !== confirmPassword) {
      toast.showError(t('auth.register.errors.passwordMismatch', { defaultValue: 'Mật khẩu không khớp' }));
      return;
    }

    if (!hasLength || !hasLower || !hasUpper || !hasNumber) {
      toast.showError(t('auth.register.errors.weakPassword', { defaultValue: 'Mật khẩu không đạt yêu cầu' }));
      return;
    }
    ...
    try {
      ...
      if (!usernameRegex.test(username.trim())) {
        toast.showError(t('auth.register.errors.username_invalid', { defaultValue: 'Tên người dùng chỉ chứa chữ, số, gạch dưới và gạch ngang' }));
        return;
      }

      if (username.trim().length < 3 || username.trim().length > 30) {
        toast.showError(t('auth.register.errors.username_length', { defaultValue: 'Tên người dùng phải từ 3 đến 30 ký tự' }));
        return;
      }
      ...
      if (!emailRegex.test(email.trim())) {
        toast.showError(t('auth.register.errors.invalid_email', { defaultValue: 'Vui lòng nhập email hợp lệ' }));
        return;
      }
      ...
      toast.showSuccess(t('auth.register.otpSent', { defaultValue: 'OTP đã được gửi đến email của bạn' }));

      // Show OTP Modal
      setShowOTP(true);
    } catch (error: any) {
      toast.showError(error.message || t('auth.register.failed', { defaultValue: 'Đăng ký thất bại' }));
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const handleResendOTP = async () => {
    ...
    try {
      ...
      toast.showError(error.message || t('auth.register.otpResendFailed', { defaultValue: 'Gửi lại OTP thất bại' }));
      throw error;
    }
  };
        ...
        <div className={`absolute top-0 h-full transition-all duration-700 ease-in-out left-0 w-1/2 z-20 ${isSignUp ? 'translate-x-[100%] opacity-0' : 'translate-x-0 opacity-100'}`}>
          <form onSubmit={handleSignInSubmit} className="bg-[#151525] flex flex-col items-center justify-center h-full px-10 text-center">
            ...
            <div className="w-full space-y-3">
              <AuthInput
                icon="account_circle"
                placeholder={t('auth.login.identifier', { defaultValue: 'Email or Username' })}
            ...
            <div className="w-full flex justify-between items-center mt-3 mb-6">
              ...
              <Link href="/auth/forgot-password" className="text-[11px] font-bold text-purple-400 hover:text-purple-300 transition-colors">{t('auth.login.forgotPassword')}</Link>
            </div>
